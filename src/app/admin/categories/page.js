    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">Manage product categories</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          [...Array(6)].map((_, i) => (
            <div key={i} className="bg-white rounded-xl border border-neutral-200/60 p-5 h-32 animate-pulse" />
          ))
        ) : categories.map((cat) => (
          <div key={cat.id} className="bg-white rounded-xl border border-neutral-200/60 shadow-sm overflow-hidden hover:shadow-md transition-all">
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-2">
                <h3 className="font-display font-semibold text-neutral-900 text-sm leading-tight">{cat.name}</h3>
                <span className="shrink-0 text-lg font-display font-bold text-neutral-300 editorial-number">
                  {cat.product_count || 0}
                </span>
              </div>
              <p className="text-xs text-neutral-500 line-clamp-2 mb-3">{cat.description}</p>
